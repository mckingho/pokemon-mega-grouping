...
fn find_min_for_type_coverage(megas: MegaPokemons) {
    ...
    for (pair_type_a, pair_type_b) in types_pairing.edges() {
        ...
        let type_a_megas: HashSet<&str> = type_mons_map.get(&type_a).unwrap().iter().cloned().collect();
        let type_b_megas: HashSet<&str> = type_mons_map.get(&type_b).unwrap().iter().cloned().collect();
...
pub fn print_per_type_ascending() {
    ...
    types.sort_by(|a, b| type_mons_map.get(a).unwrap().len().cmp(&type_mons_map.get(b).unwrap().len()));
